...
| `BUZZ_AGENT_MAX_OUTPUT_TOKENS` | `65536` | Desired per-call ceiling. OpenRouter clamps it to the catalog's explicit `top_provider.max_completion_tokens` when present; other/custom deployments must set this at or below their served model limit. Raising it reserves more of `BUZZ_AGENT_MAX_CONTEXT_TOKENS` for output. |
| `BUZZ_AGENT_MAX_TOKEN_RECOVERIES` | `5` | Retries after a successful response is truncated at the output-token limit. `0` disables recovery; the finite value and `BUZZ_AGENT_MAX_ROUNDS` prevent infinite retries. |

...
/// Reasoning summary mode for the OpenAI Responses route.
///
/// Controls the `reasoning.summary` field emitted alongside `reasoning.effort`;
/// only takes effect when `thinking_effort` is also set. Default `Auto`.
...
mod tests {
    ...
    // ---- ThinkingSummary / parse_thinking_summary ----
    ...
    // ---- anthropic_thinking_config_generated: display:"summarized" in all enabled shapes ----

    #[test]
    fn anthropic_thinking_config_generated_adaptive_emits_display_summarized() {
        ...
        for (provider, model) in &[
            ...
            let (thinking, _) =
                anthropic_thinking_config_generated(provider, model, ThinkingEffort::High, 32_768);
            let t = thinking
    ...
    #[test]
    fn anthropic_thinking_config_generated_manual_budget_emits_display_summarized() {
        ...
        for (provider, model) in &[
            ...
            let (thinking, _) =
                anthropic_thinking_config_generated(provider, model, ThinkingEffort::High, 65_536);

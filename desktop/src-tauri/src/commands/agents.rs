...
mod tombstone;
#[cfg(test)]
use tombstone::{classify_tombstone_retry, TombstoneRetry};
pub(super) use tombstone::{tombstone_managed_agent_pending, TombstoneDisposition};
...
pub async fn delete_managed_agent(
    ...
    tokio::task::spawn_blocking(move || {
        let state = app.state::<AppState>();
        {
            ...
            // Enqueue-before-erase: durably retain the authoritative tombstone
            // BEFORE any local destruction. For a relay-canonical agent a failed
            // enqueue propagates here, leaving the record fully intact and the
            // delete retryable — never erased against a missing retry.
            match tombstone_managed_agent_pending(&app, &state, &pubkey, &relay_authority)? {
                TombstoneDisposition::DeferErase { evidence } => {
                    // Relay-canonical: keep the record on disk in Deleting state.
                    // The deletion flush erases record/key and archives the
                    // identity only after verified relay confirmation, so a crash
                    // between enqueue and erase retries the byte-identical
                    // tombstone; boot reconcile treats Deleting like
                    // authoritative and never republishes the projection.
                    let record = records
                        .iter_mut()
                        .find(|record| record.pubkey == pubkey)
                        .ok_or_else(|| format!("agent {pubkey} not found"))?;
                    record.relay_authority =
                        crate::managed_agents::RelayAuthority::deleting(evidence);
                    save_managed_agents(&app, &records)?;
                }
                TombstoneDisposition::EraseNow => {
                    // Legacy kind:5: the tombstone is durably enqueued and there
                    // is no async relay confirmation, so erase in this same lock.
                    ...
                    save_managed_agents(&app, &records)?;
                    // Remove the agent's nsec from the keyring after the record
                    // is gone.
                    crate::managed_agents::delete_agent_key(&pubkey);
                    // NIP-IA: archive the deleted agent's identity on the relay so
                    // it stops appearing in member pickers and autocomplete. Same

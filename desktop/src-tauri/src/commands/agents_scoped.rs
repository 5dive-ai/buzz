//! Captured-scope agent start helpers, split from `agents.rs` (file-size
//! guard). These variants use a captured [`WorkspaceAgentScope`] so concurrent
//! workspace switches cannot redirect reads/writes to the wrong scope.

/// Spawn auto-start pairs for `pubkey` on each of `relay_urls`, using the
/// live active scope (live reads — relay and owner keys come from current
/// app state). Used by import/restore callers that don't yet hold a captured
/// scope.
///
/// Returns a full [`ManagedAgentSummary`] on success.
pub(crate) async fn start_local_agent_pairs_with_preflight(
    app: &super::AppHandle,
    state: &super::AppState,
    pubkey: &str,
    relay_urls: &[String],
) -> Result<super::ManagedAgentSummary, String> {
    let record_snapshot = {
        let _store_guard = state
            .managed_agents_store_lock
            .lock()
            .map_err(|e| e.to_string())?;
        super::load_managed_agents(app)?
            .into_iter()
            .find(|record| record.pubkey == pubkey)
            .ok_or_else(|| format!("agent {pubkey} not found"))?
    };
    if record_snapshot.backend != super::BackendKind::Local {
        return Err(format!("agent {pubkey} is not a local agent"));
    }
    let personas_for_preflight = super::load_personas(app).unwrap_or_default();
    let global_for_preflight =
        crate::managed_agents::load_global_agent_config(app).unwrap_or_default();
    let mesh_model_id =
        crate::managed_agents::effective_config::resolve_effective_relay_mesh_model_id(
            &record_snapshot,
            &personas_for_preflight,
            &global_for_preflight,
        );
    super::ensure_relay_mesh_for_record(app, mesh_model_id.as_deref(), false).await?;

    {
        let _store_guard = state
            .managed_agents_store_lock
            .lock()
            .map_err(|e| e.to_string())?;
        let mut records = super::load_managed_agents(app)?;
        let record = super::find_managed_agent_mut(&mut records, pubkey)?;
        let personas = super::load_personas(app).unwrap_or_default();
        if let Some(persona_id) = record.persona_id.clone() {
            if let Some(persona) = personas.iter().find(|persona| persona.id == persona_id) {
                crate::managed_agents::persona_events::apply_persona_snapshot(record, persona);
                record.updated_at = crate::util::now_iso();
            }
        }
        super::save_managed_agents(app, &records)?;
        if let Some(saved_record) = records.iter().find(|record| record.pubkey == pubkey) {
            super::retain_managed_agent_pending(app, state, saved_record);
        }
    }

    let mut errors = Vec::new();
    for relay_url in relay_urls {
        if let Err(error) = crate::managed_agents::start_managed_agent_runtime_pair_lazy(
            pubkey.to_string(),
            relay_url.clone(),
            app.clone(),
        ) {
            errors.push(format!("{relay_url}: {error}"));
        }
    }
    if !errors.is_empty() {
        return Err(format!(
            "failed to restart one or more managed-agent runtime pairs: {}",
            errors.join("; ")
        ));
    }

    let _store_guard = state
        .managed_agents_store_lock
        .lock()
        .map_err(|e| e.to_string())?;
    let records = super::load_managed_agents(app)?;
    let runtimes = state
        .managed_agent_processes
        .lock()
        .map_err(|e| e.to_string())?;
    let personas = super::load_personas(app).unwrap_or_default();
    let record = records
        .iter()
        .find(|record| record.pubkey == pubkey)
        .ok_or_else(|| format!("agent {pubkey} not found"))?;
    super::build_managed_agent_summary(
        app,
        record,
        &runtimes,
        &personas,
        &crate::managed_agents::load_global_agent_config(app).unwrap_or_default(),
    )
}

/// Scoped variant of [`start_local_agent_pairs_with_preflight`]: uses captured
/// `definitions_dir` for all reads and writes so a concurrent workspace switch
/// cannot redirect operations to the new scope.
///
/// Validates scope generation inside the store lock before writing, so a switch
/// between the generation check at the call site and the actual write is caught.
///
/// Called from `global_agent_config.rs` respawn path where a `definitions_dir`
/// was captured before the stop phase.
pub(crate) async fn start_local_agent_pairs_with_preflight_at(
    app: &super::AppHandle,
    state: &super::AppState,
    pubkey: &str,
    relay_urls: &[String],
    captured_scope: &crate::managed_agents::scope::WorkspaceAgentScope,
) -> Result<(), String> {
    let definitions_dir = &captured_scope.definitions_dir;

    let record_snapshot = {
        let _store_guard = state
            .managed_agents_store_lock
            .lock()
            .map_err(|e| e.to_string())?;
        crate::managed_agents::storage::load_managed_agents_at(definitions_dir)?
            .into_iter()
            .find(|record| record.pubkey == pubkey)
            .ok_or_else(|| format!("agent {pubkey} not found in captured scope"))?
    };
    if record_snapshot.backend != super::BackendKind::Local {
        return Err(format!("agent {pubkey} is not a local agent"));
    }
    let personas_for_preflight =
        crate::managed_agents::load_personas_at(definitions_dir).unwrap_or_default();
    let global_for_preflight =
        crate::managed_agents::global_config::load_global_agent_config_at(definitions_dir)
            .unwrap_or_default();
    let mesh_model_id =
        crate::managed_agents::effective_config::resolve_effective_relay_mesh_model_id(
            &record_snapshot,
            &personas_for_preflight,
            &global_for_preflight,
        );
    super::ensure_relay_mesh_for_record(app, mesh_model_id.as_deref(), false).await?;

    {
        let _store_guard = state
            .managed_agents_store_lock
            .lock()
            .map_err(|e| e.to_string())?;
        // Revalidate generation under the store lock — the workspace may have
        // switched between the preflight check above and this write.
        crate::managed_agents::scope::validate_scope_generation(captured_scope)
            .map_err(|e| format!("start_local_agent_pairs_with_preflight_at: {e}"))?;
        let mut records = crate::managed_agents::storage::load_managed_agents_at(definitions_dir)?;
        let record = super::find_managed_agent_mut(&mut records, pubkey)?;
        let personas = crate::managed_agents::load_personas_at(definitions_dir).unwrap_or_default();
        if let Some(persona_id) = record.persona_id.clone() {
            if let Some(persona) = personas.iter().find(|persona| persona.id == persona_id) {
                crate::managed_agents::persona_events::apply_persona_snapshot(record, persona);
                record.updated_at = crate::util::now_iso();
            }
        }
        crate::managed_agents::storage::save_managed_agents_at(definitions_dir, &records)?;
        if let Some(saved_record) = records.iter().find(|record| record.pubkey == pubkey) {
            super::retain_managed_agent_pending(app, state, saved_record);
        }
    }

    let mut errors = Vec::new();
    for relay_url in relay_urls {
        if let Err(error) = crate::managed_agents::start_managed_agent_runtime_pair_lazy(
            pubkey.to_string(),
            relay_url.clone(),
            app.clone(),
        ) {
            errors.push(format!("{relay_url}: {error}"));
        }
    }
    if !errors.is_empty() {
        Err(format!(
            "failed to restart one or more managed-agent runtime pairs: {}",
            errors.join("; ")
        ))
    } else {
        Ok(())
    }
}

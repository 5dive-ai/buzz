...
fn aggregate_compare_and_clear_cannot_ack_a_newer_attempt() {
    ...
    assert!(mark_managed_agent_aggregate_synced(&conn, "owner", "agent", 1, "event-1").unwrap());
    ...
    assert!(mark_managed_agent_aggregate_synced(&conn, "owner", "agent", 2, "event-2").unwrap());
    assert!(
        !get_retained_managed_agent_aggregate(&conn, "owner", "agent")
            .unwrap()
            .unwrap()
...
fn aggregate_error_is_scoped_to_the_exact_attempt() {
    ...
    .unwrap();
    assert!(
        !record_managed_agent_aggregate_error(&conn, "owner", "agent", 1, "wrong", "nope").unwrap()
    );

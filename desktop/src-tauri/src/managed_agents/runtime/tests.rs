...
#[test]
fn internal_summary_access_clamps_stale_record() {
    let rec = fixture(RespondTo::Anyone, vec!["a".repeat(64)], Some("tag".into()));

    let (respond_to, allowlist) = super::summary_access_with_policy(&rec, true);

    assert_eq!(
        respond_to,
        RespondTo::OwnerOnly,
        "internal summary widened stale access"
    );
    assert!(
        allowlist.is_empty(),
        "internal summary retained a stale allowlist"
    );
}

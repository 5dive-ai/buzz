...
test("runIdleReloadCheck_os_idle_rejection_surfaces_as_null_via_hook", async () => {
  // The hook maps a rejected idle query to null; the policy then withholds.
  // Model that contract here: null in, no reload.
  const { readers, calls } = greenReaders({
    getOsIdleMs: async () => null,
    getHuddleActive: async () => {
      throw new Error("must not be reached — null idle skips huddle read");
    },
  });
  // getHuddleActive is awaited after getOsIdleMs; a throw would reject the
  // check. Prove null idle alone withholds without depending on huddle.
  await assert.rejects(runIdleReloadCheck(readers));
  assert.equal(calls.reload, 0);
});

...
// from the relay itself (entityTitleLoader), so they must render for
// recipients even when the message carries no link-preview tags.
...
test("agent-style message with bare buzz:// links renders entity cards without snapshot tags", async ({
  ...
  // The PR card resolves its title from the relay event's subject tag.
  ...
  // The repo card renders on its d-tag fallback title (repo announcements
  // have no title lookup).

...
-- ── Private managed-agent authority (NIP-PMA) ────────────────────────────────
...
CREATE TABLE managed_agent_heads (
    ...
    definition_d TEXT, definition_revision BIGINT, definition_event_id BYTEA,
    definition_content_sha256 BYTEA, instance_event_id BYTEA, instance_content_sha256 BYTEA,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (community_id, owner_pubkey, agent_pubkey),
    UNIQUE (community_id, event_id),
    CHECK ((state = 'active') = (definition_d IS NOT NULL AND definition_revision IS NOT NULL AND definition_event_id IS NOT NULL AND definition_content_sha256 IS NOT NULL AND instance_event_id IS NOT NULL AND instance_content_sha256 IS NOT NULL)),
...
CREATE INDEX managed_agent_heads_definition_binding ON managed_agent_heads (community_id, owner_pubkey, definition_d);
CREATE TABLE managed_agent_revisions (
    community_id UUID NOT NULL, owner_pubkey BYTEA NOT NULL, agent_pubkey BYTEA NOT NULL,
    generation BIGINT NOT NULL CHECK (generation > 0 AND generation <= 9007199254740991),
    event_id BYTEA NOT NULL CHECK (length(event_id) = 32),
    ...
    UNIQUE (community_id, event_id),
    FOREIGN KEY (community_id, owner_pubkey, agent_pubkey) REFERENCES managed_agent_heads (community_id, owner_pubkey, agent_pubkey) DEFERRABLE INITIALLY DEFERRED
);
